...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        if (ev.type == ALLEGRO_EVENT_TIMER) {
            ...
            if (play) {
                //environment code
                double observation[] = {cactuses[player.getScore()].getX() - player.getRight(),
                                        cactuses[player.getScore()].getBottom() - player.getTop() - Player::SIZE};
                bool action = 0;
                ...
                if (player.getRight() >= cactuses[player.getScore()].getX() &&
                    player.getLeft() <= cactuses[player.getScore()].getX() + Cactus::WIDTH) {
                    ...
                    int bottom_cactus_middle_y =
                            top_cactus_middle_y + cactuses[player.getScore()].getGap() + Cactus::WIDTH;
                    ...
                    if (player_middle_y >= top_cactus_middle_y && player_middle_y <= bottom_cactus_middle_y) {
                        ...
                        double distance_top = sqrt(
                                (player_middle_x - top_cactus_middle_x) * (player_middle_x - top_cactus_middle_x) +
                                (player_middle_y - top_cactus_middle_y) * (player_middle_y - top_cactus_middle_y));
                        //distance between centers bottom
                        double distance_bottom = sqrt(
                                (player_middle_x - top_cactus_middle_x) * (player_middle_x - top_cactus_middle_x) +
                                (player_middle_y - bottom_cactus_middle_y) *
                                (player_middle_y - bottom_cactus_middle_y));
        ...
        if (redraw && al_is_event_queue_empty(event_queue)) {
            ...
            al_draw_bitmap(grass_bitmap, GRASS_WIDTH + ((int) map_offset % GRASS_WIDTH), SCREEN_HEIGHT - GRASS_HEIGHT,
                           0);
            player.draw(map_offset);

            //draw score
            if (player.getScore() == CACTUS_BUFFER_SIZE) {
                al_draw_text(font, al_map_rgb(205, 50, 50), SCREEN_WIDTH - (FONT_SIZE / 8), 0, ALLEGRO_ALIGN_RIGHT,
                             "WIN");
            } else {
                al_draw_textf(font, al_map_rgb(255, 255, 255), SCREEN_WIDTH - (FONT_SIZE / 8), 0, ALLEGRO_ALIGN_RIGHT,
                              "%i", player.getScore());
            }
